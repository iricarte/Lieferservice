...
public record ProblemGroupImpl(List<ProblemArchetype> problems,
     ...
     * @param ratingCriteria The {@link RatingCriteria}s used to evaluate the problems in this
     *                       {@link ProblemGroupImpl}.
     */
    ...
    public ProblemGroupImpl {
        ...
        for (ProblemArchetype problem : problems) {
            if (!problem.raterFactoryMap().keySet().containsAll(ratingCriteria)) {
                throw new IllegalArgumentException("Problem %s has no rater registered for rating criterion %s".formatted(
                        problem.toString(),
                        ratingCriteria.stream()
