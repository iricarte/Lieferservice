...
class RegionBuilderImpl implements Region.Builder {
    ...
    private final Set<EdgeBuilder> edges = new TreeSet<>(Comparator.comparing(EdgeBuilder::getLocationA)
                                                                   .thenComparing(EdgeBuilder::getLocationB));
    ...
    public Region.Builder removeComponent(String name) {
        ...
        for (NodeBuilder nodeBuilder : nodes.values()) {
            if (nodeBuilder.name.equals(name)) {
                nodes.remove(nodes.keySet()
                                  .stream()
                                  .filter(key -> nodes.get(key).equals(nodeBuilder))
                                  .findFirst()
                                  .orElseThrow());
                return this;
            }
    ...
    public Region build() {
        ...
        nodes.forEach((l, n) -> {
            n.connections = new HashSet<>(); //reset connection to not modify nodes created by previous
            // calls to build()
        ...
        edges.forEach(e -> {
            ...
            if (!nodes.containsKey(e.getLocationA()) || !nodes.containsKey(e.locationB)) {
                throw new IllegalArgumentException(
                        "Can't create an edge if one of the connected nodes wasn't added to the " + "region");
            }
