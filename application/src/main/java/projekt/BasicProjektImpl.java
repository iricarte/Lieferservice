...
public class BasicProjektImpl implements Projekt {
    ...
    public void start() {
        ...
        raterFactoryMap1.put(RatingCriteria.IN_TIME,
                             InTimeRater.Factory.builder().setIgnoredTicksOff(5).setMaxTicksOff(25).build());

        raterFactoryMap1.put(RatingCriteria.TRAVEL_DISTANCE,
                             TravelDistanceRater.Factory.builder()
                                                        .setFactor(0.75)
                                                        .setVehicleManager(vehicleManager1)
                                                        .build());

        raterFactoryMap1.put(RatingCriteria.AMOUNT_DELIVERED,
                             AmountDeliveredRater.Factory.builder().setFactor(0.99).build());

        Map<RatingCriteria, Rater.Factory> raterFactoryMap2 = new HashMap<>();
        raterFactoryMap2.put(RatingCriteria.IN_TIME,
                             InTimeRater.Factory.builder().setIgnoredTicksOff(5).setMaxTicksOff(25).build());

        raterFactoryMap2.put(RatingCriteria.TRAVEL_DISTANCE,
                             TravelDistanceRater.Factory.builder()
                                                        .setFactor(0.75)
                                                        .setVehicleManager(vehicleManager2)
                                                        .build());

        raterFactoryMap2.put(RatingCriteria.AMOUNT_DELIVERED,
                             AmountDeliveredRater.Factory.builder().setFactor(0.99).build());

        //ProblemArchetype
        ProblemArchetype problemArchetype1 = new ProblemArchetypeImpl(orderGeneratorFactory1,
                                                                      vehicleManager1,
                                                                      raterFactoryMap1,
                                                                      simulationLength,
                                                                      "problem 1");
        ProblemArchetype problemArchetype2 = new ProblemArchetypeImpl(orderGeneratorFactory2,
                                                                      vehicleManager2,
                                                                      raterFactoryMap2,
                                                                      simulationLength,
                                                                      "problem 2");
        ...
        //ProblemGroup
        ProblemGroup problemGroup = new ProblemGroupImpl(List.of(problemArchetype1, problemArchetype2),
                                                         new ArrayList<>(raterFactoryMap1.keySet()));

        new RunnerImpl().run(problemGroup, simulationConfig, 1, BasicDeliveryService::new, (simulation, problem, i) -> {
        }, (simulation, problem) -> false, result -> {
