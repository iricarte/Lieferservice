...
import static projekt.util.Utils.createOccupiedRestaurant;
...
public class TutorTests_H8_AmountDeliveredRaterTest {
    ...
    public void testNoOrdersDelivered(double factor, double expected) {
        Rater amountDeliveredRater1 = AmountDeliveredRater.Factory.builder().setFactor(factor).build().create();

        Context context1 = contextBuilder().subject("amountDeliveredRater#getScore")
        ...
        amountDeliveredRater1.onTick(List.of(OrderReceivedEvent.of(0, order1), OrderReceivedEvent.of(0, order2)), 0);

        amountDeliveredRater1.onTick(List.of(OrderReceivedEvent.of(1, order3), OrderReceivedEvent.of(1, order4)), 1);

        assertEquals(expected, amountDeliveredRater1.getScore(), context1, TR -> "method did not return correct score");

        Rater amountDeliveredRater2 = AmountDeliveredRater.Factory.builder().setFactor(factor).build().create();

        Context context2 = contextBuilder().subject("amountDeliveredRater#getScore")
    ...
    public void testAllOrdersDelivered(double factor, double expected) {
        ...
        Rater amountDeliveredRater = AmountDeliveredRater.Factory.builder().setFactor(factor).build().create();

        Context context = contextBuilder().subject("amountDeliveredRater#getScore")
                                          .add("orders received", 4)
        ...
        amountDeliveredRater.onTick(List.of(OrderReceivedEvent.of(0, order1), OrderReceivedEvent.of(0, order2)), 0);

        order1.setActualDeliveryTick(1);
        order2.setActualDeliveryTick(1);

        amountDeliveredRater.onTick(List.of(OrderReceivedEvent.of(1, order3),
        ...
        amountDeliveredRater.onTick(List.of(DeliverOrderEvent.of(2, vehicle, neighborhood, order3),
    ...
    public void testSomeOrdersDelivered(double factor, double expected1, double expected2) {
        ...
        amountDeliveredRater.onTick(List.of(OrderReceivedEvent.of(1, order3),
                                            OrderReceivedEvent.of(1, order4),
                                            DeliverOrderEvent.of(1, vehicle, neighborhood, order1)), 1);
        ...
        assertTrue(Math.abs(expected1 - amountDeliveredRater.getScore()) < 0.001,
                   context1,
                   TR -> "method did not return correct score. Expected %f but was %f".formatted(expected1,
                                                                                                 finalAmountDeliveredRater.getScore()));

        amountDeliveredRater = AmountDeliveredRater.Factory.builder().setFactor(factor).build().create();
        ...
        amountDeliveredRater.onTick(List.of(OrderReceivedEvent.of(1, order3), OrderReceivedEvent.of(1, order4)), 1);

        order3.setActualDeliveryTick(2);

        amountDeliveredRater.onTick(List.of(DeliverOrderEvent.of(2, vehicle, neighborhood, order3)), 2);

        Rater finalAmountDeliveredRater1 = amountDeliveredRater;
        assertTrue(Math.abs(expected2 - amountDeliveredRater.getScore()) < 0.001,
                   context2,
